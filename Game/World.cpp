...
bool World::SerializeOut( const std::string & strPath )
{
	...
	std::ofstream jsonFile( strFullPath.c_str(), std::ios_base::out | std::ios_base::binary | std::ofstream::trunc );
	if ( !jsonFile.is_open() )
	{
		GASSERT( false );
		return false;
...
bool World::SerializeIn( const std::string & strPath )
{
	...
	for ( auto& it : entities )
	{
		...
		if ( pEnt->GetID() >= m_NextID )
			m_NextID = pEnt->GetID() + 1;
	}
...
void World::RemoveEntity( BaseEntity * pEntity )
{
	if ( pEntity )
	{
...
void World::RemoveEntity( unsigned int uID )
{
	for ( auto& it = m_EntityList.begin(); it != m_EntityList.end(); it++ )
	{
		if ( ( *it )->GetID() == uID  )
